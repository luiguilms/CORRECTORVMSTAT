...
function correctVmstatFile(content) {
  ...
  for (let i = 1; i < blocks.length; i++) {
    if (
      blocks[i].headerLines.length === 0 &&
      blocks[i - 1].headerLines.length > 0
    ) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (
      originalDataCount !== 3 ||
      hasDataMoved(originalBlock.dataLines, assignedData)
    ) {
      const change = {
        ...
        endLine:
          ...
          originalBlock.dateLines.length +
          originalBlock.headerLines.length +
          originalDataCount -
          1,
        ...
        affectedLines: `Líneas ${originalBlock.startLine} - ${
          originalBlock.startLine +
          originalBlock.dateLines.length +
          originalBlock.headerLines.length +
          originalDataCount -
  ...
  return {
    ...
    stats: {
      ...
      dataLinesRedistributed: changes.reduce(
        (sum, change) =>
          sum + Math.abs(change.originalDataCount - change.finalDataCount),
        0
      ),
    },
  };
...
function correctVmstatRamFile(content) {
  ...
  blocks.forEach((block) => {
    const totalLines =
      block.dateLines.length +
      block.headerLines.length +
  ...
  const originalDataCount = blocks.reduce(
    (sum, block) => sum + block.dataLines.length,
    0
  );
  ...
  while (
    date1Index < allDate1Lines.length ||
...
function isDateLine2(line) {
  return /^(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+\w+\s+\d+\s+\d{2}:\d{2}:\d{2}\s+-\d{2}\s+\d{4}$/.test(
    line.trim()
  );
}
